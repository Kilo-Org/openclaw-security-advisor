...
- `kilocode_security_advisor` tool description now includes an update-reminder hint: the agent will occasionally (not every invocation) suggest the user check for a newer plugin version with `npm view @kilocode/openclaw-security-advisor version` and upgrade via `openclaw plugins install @kilocode/openclaw-security-advisor && openclaw gateway restart` when they're behind.
- README has a new `Staying up to date` section with the version-check and upgrade commands, plus a note that the agent will periodically prompt for this.
